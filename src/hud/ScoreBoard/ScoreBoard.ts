...
export class ScoreBoard extends Container {
  ...
  constructor(initialBalance: number, initialBet: number) {
    ...
    this.balanceValue = new Text({ text: initialBalance.toString(), style: { ...valueStyle, fill: 0xffcc00 } });
    
    const betLabel = new Text({ text: "BET:", style: labelStyle });
    this.betValue = new Text({ text: initialBet.toString(), style: valueStyle });

    [balanceLabel, this.balanceValue, betLabel, this.betValue].forEach(t => t.anchor.set(0, 0.5));
    ...
    this.addChild(this.bg, balanceLabel, this.balanceValue, betLabel, this.betValue);
  }
